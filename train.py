...
def getActivations(x_train, numActivationTrainingInstances, dnnModel, y_train):
    ...
    if len(mapNewYValues) == 0:
        ...
        # y_train is the sub-classes, change it to the superclasses
        y_train_mapped = util.transformDataIntoZeroIndexClasses(y_train, mapOriginalYValues, mapNewYValues)
        # with mapped data, the original classes are mapped to classes starting from zero
        # so no transformation needs to take place
    ...
    for batchIndex in range(numBatches):
        ...
        activations, numLayers = extract.getActivations(dnnModel, batch)
